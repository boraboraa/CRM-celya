#!/usr/bin/env bash
# Assemble la recette différentielle de la 023 en UN script, à exécuter d'un
...
# rien n'est validé — la tâche pg_cron planifiée par la migration comprise.
set -euo pipefail
cd "$(dirname "$0")/../.."
out="${1:-/dev/stdout}"
{
...
SQL
  echo "-- === Migration 023, verbatim ==="
  cat supabase/migrations/023_fiche_close.sql
  echo
  echo "-- === Assertions ==="
  cat supabase/recettes/023_corps.sql
  cat <<'SQL'

-- === Fin : le rapport, et le ROLLBACK garanti ===
do $fin$
begin
  raise exception 'RECETTE 023 — % OK, % FAUTE(S)%',
